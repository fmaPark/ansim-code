...
| 표준 | 조항 | 구현 대상 |
| --- | --- | --- |
| TTAK.KO-11.0309/R1 (SBOM 속성 규격) | §5.2, **§6 전체**(15속성별 확인 절차), §7.2 표 7-1, §7.3, §7.4 | 15속성 SBOM 출력 스키마(§6.2~6.15의 속성별 확인 절차 구현), 스캐닝 도구 자체(§6.1·§7.3 — 표준이 도구 구축을 직접 요구), SBOM 목록 업데이트=재진단(§7.4, §7.2 표 7-1 운영·유지관리) |
| TTAK.KO-11.0259/R1 (보안취약점 관리 지침) | **§9.3**(설계), **§9.5**(테스트), §9.6(유지보수), §10, **§11**(관리 프로세스) | §9.3 ← 미선언 의존성 검출, 감사보고서형 리포트(플래그 파일마다 해결법). §9.5 ← **주석 내 부적절한 내용(시크릿·내부 정보) 검사**, 배포 전 패키지 진단(서비스 사용 시점의 근거). §9.6 ← 주기적 점검=재진단. §10 ← 조직 구성 체크리스트. §11 ← 진단 파이프라인 5단계 매핑(§4.1), **§11.3 "위험 평가 지수" = 안전등급의 표준 근거**(§4.5) |
| TTAK.KO-12.0414 (AI 서비스 개인정보보호) | §7.3, **§7.3.1**(6대 원칙) | 개인정보 생명주기 진단 룰 **10종**(§4.5). §7.3.1 투명성 원칙 ← P9(처리방침 공개). **§7.1~7.2 및 §7.3 중 조직·물리적 조치 요구사항**(내부 관리계획·물리적 보안장치 등)은 체크리스트 안내 |
| TTAK.KO-11.0322 (SBOM 거버넌스) | §5.1~5.2, **§5.1.2 표 5-1** | 공급망 환경 자동 분류(자체개발·오픈소스·바이너리, §5.1.1), 비즈니스 모델별 위험요인 매트릭스 리포트 출력(§5.1.2 표 5-1) + **AGPL/SSPL 서비스 배포 경고 룰**, 재진단(§5.2 지속적 SBOM 갱신) |
...
- 취약점 대조: **OSV.dev API** + **KISA 보호나라 공공데이터 CSV 스냅샷** — 공지 본문 CVE 추출·교차, "국내 보안공지 발령" 표시(가산점 항목)
...
| --- | --- | --- |
| §11.1 환경 분석 | 진단 대상 선정, 수행 계획 수립 | 입력 접수·전개·검증, 진단 범위 결정 |
| §11.2 현황 진단 | 구성요소 목록 및 **갭 분석 보고서** | SBOM 생성 + 미선언 의존성 검출 |
| §11.3 위험 분석 | **취약점 진단서 및 위험 평가 지수** | 취약점 대조 + 정적 룰 + 안전등급 산정 |
| §11.4 대책 수립 | 보안 대책 도출, 이행 계획 | 수정 프롬프트 생성, 리포트 |
| §11.5 이행 관리 | 점검결과서, **취약점 이력관리 대장** | 재진단 + 스캔 이력 연결(`previous_scan_id`) |
...
| KISA 보호나라 KrCERT 게시판 ([data.go.kr](https://www.data.go.kr/data/15155789/fileData.do)) | 공공데이터 | 보안공지 **본문에서 CVE ID 추출 → OSV 결과와 교차**, 교차 취약점에 "국내 보안공지 발령" 표시 + 공지 링크 노출(가산점 항목의 실질 활용). 데이터셋 최종 확정은 M3 실데이터 확인 후(§11 항목 5) | 공공누리. CSV 스냅샷을 이미지에 동봉 |
...
| 3 | 시크릿 플레이스홀더 allowlist 보강(오탐률 실측 기반) | 풀스택 | M4 |
| 4 | 주민번호 체크섬 무효 처리(review_needed 기본값)의 기획 최종 확인 | 기획 | M4 전 |
| 5 | KISA 공공데이터 데이터셋(data.go.kr/15155789) 최종 확정 | 풀스택 | M3 |
