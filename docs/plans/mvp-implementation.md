...
**Interfaces:**
- Produces: `ansim-benchmark` 구조 — `vulnerable/`(기획 확정 목록의 취약점을 심은 Python·JS 파일들 + 취약 버전 `requirements.txt`·`package-lock.json`), `clean/`(오탐 측정용 — 유사하지만 안전한 코드: 파라미터 바인딩 쿼리, 플레이스홀더 키, 암호화 저장), 루트 `README.md`(취약점 의도 명세). **스키마·매칭 사양은 [benchmark-spec §5.1·§5.2](../benchmark-spec.md)가 우선한다**(개발이 룰에 맞춰 수정 금지, 순환 검증 회피). `expected_findings.yaml` 항목은 `{rule_id, verdict, note}` + **룰 유형별 키**(파일 키 룰=`file`(+스캐폴딩 후 `line`), 컴포넌트 키 룰 SCA-01~09=`package`, declared_in 키 룰 SCA-10~12=`file`(매니페스트), repo-wide 룰 P8·P9·P1=`file: null`). `measure_detection.py`: 벤치마크를 API로 스캔 → finding과 expected를 **§5.1의 4종 키로 매칭**(SCA는 evidence에서 컴포넌트명 파싱, 대표 키잉+다발 허용, 오라클 외 confirmed는 '부가 발견'으로 TPR·FPR 제외) → 룰별 `TPR = 검출/기대`, `FPR = clean/ 파일에서 발생한 confirmed / clean 파일 수` 산출 → `docs/measurements.md`에 표 append(전체 31종 기준 — 미시연 룰도 표에 포함, TDD §9 ③).
