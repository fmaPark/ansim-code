...
* **Update**: `tdd.md`(§아키텍처 다이어그램·기술 스택 표) / `plans/mvp-implementation.md`(Tech Stack) — 프론트엔드 표기를 React 18 → **React 19.2**로 정정. 실제 구현이 `web/package-lock.json` 기준 react·react-dom 19.2.8이라 설계 문서 표기와 어긋나 있었다. 문서 버전(TDD v0.4)은 올리지 않았다 — 설계 판단 변경이 아니라 실물과의 표기 불일치 정정이다.
