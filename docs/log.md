...
* **Creation**: `ai-usage.md` — 개발 프로세스의 AI 활용 정리. **제출문서 작성자가 참고하는 문서**이므로 맨 앞에 요약(숫자 표 + 특징 5가지)을 두고 세션 전수 나열 대신 대표 7건만 실었다. 근거는 `logs/agent/` 세션 백업 23건(2026-08-24~08-31)과 PR 27건·이슈 14건·커밋 123건. ①전체 워크플로우(설계→계획→프롬프트→실행 4단 문서 체인, 프롬프트 전담 세션 「프롬프트 발사대」, 세션=마일스톤=브랜치=PR 분할, G1~G16을 프롬프트가 아닌 문서에 두는 가드레일, 계획=Fable 5 / 구현=Opus 5 고정 규칙과 커밋 귀속 표기, 세션 기록 자동화와 저장소 규약 정비, 이슈 트래커=다음 세션 입력) ②대표 세션 7건 + 구현 세션 공통 틀 + 사람 개입 지점 ③AI 결과 검증(정답지 기획 선확정 게이트, 자기진단 오탐 3종, 제품 LLM 통제와 인젝션 4종 실증, 기록으로 남은 실패 5건). 번들 밖 로그를 근거로 삼으므로 `sources`에 그 사실을 명시한다. §4는 기획 확인 필요 2건(claude.ai 08-24~08-28 협의 구간, 기획 쪽 PR 리뷰 작성 방식)과 개발 쪽 결손(PR #37·#41 세션 미백업). 부록에 **사례별 프롬프트 원문 8종**(설계 문서 생성 / 계획 생성 / 구현 세션 / 병렬 실행·모델 지정 / 문서 갱신 / 프롬프트 보강 / 세션 인계 / 짧은 개입)을 실어, 서술만으로는 전달되지 않는 지시 방식을 그대로 보게 했다. `index.md`에 「프로세스」 절 신설.
