...
| 지표 | M7 실측 | 수정 후 | 비고 |
| --- | --- | --- | --- |
| TPR | 48/51 (94.1%) | **49/51 (96.1%)** | AUX-03 0% → **100%**(#31) |
...
## 후속 이슈 처리 2회차 — #34 잔여분 수정, #33·#30 방침 확정 (2026-08-30)

심각도 평가와 선택 근거는 [이슈 #30·#33·#34 심각도 평가와 해결 계획](plans/2026-08-30-issues-30-33-34-remediation.md)에 있다.
등급 기여 여부와 오류 방향으로 #34 High·#33 Medium·#30 Low로 매기고, #34만 코드를 고쳤다.

### ① P9 회복 — 자기진단에서 정탐 발화 (#34 종결)

위 「③ 남은 관찰」이 지목한 라우트 검사를 두 방향에서 좁혔다. **둘이 함께 있어야 닫힌다** —
정규식만으로는 테스트 픽스처가, 경로 필터만으로는 `semgrep_runner.py`가 남는다.

| 변경 | 내용 |
| --- | --- |
| 정규식 | `['\"]/?privacy` → `['\"]/?privacy(?:[-_]?policy)?['\"/]` — 라우트 종결 문자(따옴표·슬래시)를 요구한다. 라우트는 privacy에서 끝나거나 세그먼트가 이어지지만 파일명은 점이 이어진다 |
| 경로 | 라우트 증거를 비테스트 파일로 한정. #29에서 만든 `config.is_test_path`를 그대로 쓴다(새 함수 없이 호출부만 증가) |

수정 전후로 저장소 전체를 세어 확인했다.

| 조건 | P9를 끄는 파일 |
| --- | --- |
| 수정 전 | `semgrep_runner.py` · `test_privacy_rules.py` · `test_verification_matching.py` |
| 정규식만 | `test_verification_matching.py` (1건) |
| 정규식 + 경로 | **0건** |

엔진 실행으로 재확인: `run_repo_checks(저장소 루트)` → `{'P5': 2, 'P9': 1}`.
**P9가 medium·confirmed로 발화한다** — 안심코드에 처리방침 문서가 실제로 없으므로 정탐이다.
P9는 등급 계산에 들어가므로 자기진단 등급 표시가 바뀔 수 있다.

`check_invariants.py`는 엔진과 1:1 계약이라 정규식·경로 필터를 함께 미러링했다.
벤치마크 체크아웃 대상 실행에서 불변식 6종 통과, api 테스트 180건 green.

**남는 한계(문서화 후 종결)**: 비테스트 코드의 라우트 아닌 문자열 상수
(`PRIVACY_URL = "/privacy"`)는 여전히 룰을 끈다. 문자열 리터럴과 라우트 선언의 구분은
구문 기반 판정이 필요해 #30과 같은 층위이며, semgrep 이관(V2) 대상으로 넘긴다.

### ② SCA-05·SCA-07 — 실효 29종임을 명시한다 (#33 방침 확정)

이슈 본문의 선택지 ①(그대로 두고 명시)을 채택했다. 레지스트리 원격 조회 도입은 캐시·타임아웃·
부분 결과 정책이 따라오는 설계 가정 변경이라 MVP 범위 밖이다. 판정식은 정상이고 입력이 없는
것이므로, 고칠 것은 코드가 아니라 **"31종"이라는 표기**다. `README.md` 「검증」에 실효 29종과
그 사유를 적었다.

`rules/catalog.yaml`은 건드리지 않았다. `rules/` 콘텐츠 해시가 `rule_catalog_version`이라
주석 한 줄이 룰 버전을 바꾸고 데모의 재진단 diff에 "룰 버전 변경"으로 나타난다 —
커버리지 사실은 문서에 적는 편이 낫다. `rule_catalog_version`은 `04a38658f614ed40` 유지.

### ③ 룰 소스 자기 발화 — 기록으로 닫는다 (#30 방침 확정)

이슈 코멘트의 선택지 ③(그대로 두고 문서화)을 채택했다. ①(제외 경로)은 닿지 않는다 —
`repo_checks.py`는 테스트 파일이 아니라 룰 소스 자신이고, P4·P5는 이미 `review_needed`라
강등할 여지가 없다. ②(semgrep 이관)는 P5·P7·P10을 구문 기반으로 옮기는 큰 변경이라
제출 전에 넣을 수 없다.

이번 회차 자기진단에서도 P5 2건이 그대로 재현된다.

| 파일 | 성격 |
| --- | --- |
| `api/app/engine/repo_checks.py` | 룰 소스가 자기 정규식 리터럴에 걸렸다 |
| `api/tests/test_privacy_rules.py` | 테스트 픽스처 문자열 |
