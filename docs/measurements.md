...
- **gitleaks·PII 절대경로 file_path(M5 이월)** — 이 세션 미수정(사용자 지시: 다른 워크트리 선머지 예정).
  이번 브랜치는 해당 파일 미변경이라 충돌 없음. 재확인 실측: 시크릿 zip 스캔에서 SEC-04·05
  `file_path=/tmp/ansim-scan-*/src/...` 노출(diff 해결/신규 목록에도 그대로). 머지 후 diff 화면 재검증 필요.
