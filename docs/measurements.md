...
> **주의: M7(Task 26·27) 실측치와 겹친다.** 위 M7 절은 `rule_catalog_version=8e512d222d972ed9`,
> `KISA-CSV@2026-08-29`(표본 스냅샷)로 측정한 값이다. 이 변경으로 **SCA-03 관련 수치는 무효**다 —
> 아래 "M7 재측정 필요" 참고. 나머지 룰의 TPR·FPR은 영향받지 않는다(SCA-03만 KISA 스냅샷에 의존).
...
- **M7 재측정 필요(신규, 이 변경이 만든 것)** — M7의 SCA-03 실측치 2건이 표본 스냅샷 기준이라 무효다:
  ① 벤치마크 TPR 표의 `SCA-03 5/5 100%` ② PyGoat dogfooding의 `SCA-03 3건`.
  실데이터로는 벤치마크 5핀 중 **django 1건만** 발화한다(오라클을 그대로 두면 TPR 1/5).
  PyGoat은 Django 앱이라 발화 자체는 유지되나 건수는 재측정해야 한다. 재측정은
  `verification/measure_detection.py`를 새 스냅샷 이미지로 1회 더 돌리면 된다.
